function [gsmval,grad,grad2]=gradhessgsmnew(imax,nbblocks,N,delta,v,Y,T,k,d,H,Hip1,Him1,Hjp1,Hjm1)
...
Ivecarr=zeros(k,(2*N+1)^2*d23*3);
Jvecarr=zeros(k,(2*N+1)^2*d23*3);
Vvecarr=zeros(k,(2*N+1)^2*d23*3);
...
%   gsmval=gsmval+((v-priormean)*priorprecmx*(v-priormean)')/2;
% 
%   grad=grad+(v-priormean)*priorprecmx;
% 
%   grad2=grad2+priorprecmx;
%    
